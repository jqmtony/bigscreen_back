...
//校验是否是正整数 fengzw
XFrameUtils.isPInteger = function (str) {
	var r=/^\+?[1-9][0-9]*$/;//正整数     
  return  r.test(str); 
};
 ...
 * @author 冯志文 fengzw
 */
XFrameUtils.isPFloat=function(str){
 ...
 XFrameUtils.clearForm = function(formId)
 {
	...
	if(objForm == null || objForm == undefined) return false;
 	...
 	for(var i=0; i<elements.length; i++)
 	{
 		var type = elements[i].type;
 		var tag = elements[i].tagName.toLowerCase();
 		if((type == "text" || type == "password" || type == "file" || tag == 'textarea' || type == "hidden") && !elements[i].readOnly)
 		{
 			elements[i].value = "";
 		}
 		//当对象为下拉列表时，清除时置为第一个值
